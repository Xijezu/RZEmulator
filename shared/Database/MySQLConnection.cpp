...
bool MySQLConnection::Execute(PreparedStatement *stmt)
{
    ...
    uint32_t index = stmt->m_index;
    {
        ...
        NG_LOG_DEBUG("sql.sql", "[%u ms] SQL(p): %s", getMSTimeDiff(_s, getMSTime()), m_mStmt->getQueryString(m_queries[index].first).c_str());
...
bool MySQLConnection::_Query(PreparedStatement *stmt, MYSQL_RES **pResult, uint64_t *pRowCount, uint32_t *pFieldCount)
{
    if (!m_Mysql)
        return false;
...
bool MySQLConnection::_Query(const char *sql, MYSQL_RES **pResult, MYSQL_FIELD **pFields, uint64_t *pRowCount, uint32_t *pFieldCount)
{
    if (!m_Mysql)
        return false;
...
uint32_t MySQLConnection::GetLastError()
{
...
MySQLPreparedStatement *MySQLConnection::GetPreparedStatement(uint32_t index)
{
    ...
    if (!ret)
        NG_LOG_ERROR("sql.sql", "Could not fetch prepared statement %u on database `%s`, connection type: %s.", index, m_connectionInfo.database.c_str(),
            (m_connectionFlags & CONNECTION_ASYNC) ? "asynchronous" : "synchronous");

    return ret;
}

void MySQLConnection::PrepareStatement(uint32_t index, const char *sql, ConnectionFlags flags)
{
...
bool MySQLConnection::_HandleMySQLErrno(uint32_t errNo, uint8_t attempts /*= 5*/)
{
    ...
    case CR_CONN_HOST_ERROR: {
        ...
        if (!lErrno) {
            ...
            NG_LOG_INFO("sql.sql", "Successfully reconnected to %s @%s:%s (%s).", m_connectionInfo.database.c_str(), m_connectionInfo.host.c_str(), m_connectionInfo.port_or_socket.c_str(),
                (m_connectionFlags & CONNECTION_ASYNC) ? "asynchronous" : "synchronous");
    ...
    case ER_NO_SUCH_TABLE:
        ...
        return false;
    case ER_PARSE_ERROR:
        NG_LOG_ERROR("sql.sql", "Error while parsing SQL. Core fix required.");
        std::this_thread::sleep_for(std::chrono::seconds(10));
        std::abort();
        return false;
    default:
        NG_LOG_ERROR("sql.sql", "Unhandled MySQL errno %u. Unexpected behaviour possible.", errNo);
        return false;
    }
}
