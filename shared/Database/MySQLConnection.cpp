...
bool MySQLConnection::Open()
{
    ...
    if (!mysqlInit)
    {
        MX_LOG_ERROR("sql.sql", "Could not initialize Mysql connection to database `%s`", m_connectionInfo.database.c_str());
        return false;
    }
    ...
    if (m_Mysql)
    {
        if (!m_reconnecting)
        {
            MX_LOG_INFO("sql.sql", "MySQL client library: %s", mysql_get_client_info());
            MX_LOG_INFO("sql.sql", "MySQL server ver: %s ", mysql_get_server_info(m_Mysql));
            // MySQL version above 5.1 IS required in both client and server and there is no known issue with different versions above 5.1
            // if (mysql_get_server_version(m_Mysql) != mysql_get_client_version())
            //     MX_LOG_INFO("sql.sql", "[WARNING] MySQL client/server version mismatch; may conflict with behaviour of prepared statements.");
        }

        MX_LOG_INFO("sql.sql", "Connected to MySQL database at %s", m_connectionInfo.host.c_str());
    ...
    {
        MX_LOG_ERROR("sql.sql", "Could not connect to MySQL database at %s: %s\n", m_connectionInfo.host.c_str(), mysql_error(mysqlInit));
...
bool MySQLConnection::Execute(const char* sql)
{
...

    {
        ...
        if (mysql_query(m_Mysql, sql))
        {
            ...
            MX_LOG_INFO("sql.sql", "SQL: %s", sql);
            MX_LOG_ERROR("sql.sql", "[%u] %s", lErrno, mysql_error(m_Mysql));
            ...
            MX_LOG_DEBUG("sql.sql", "[%u ms] SQL: %s", getMSTimeDiff(_s, getMSTime()), sql);
    }
...
bool MySQLConnection::Execute(PreparedStatement* stmt)
{
    ...
    uint32 index = stmt->m_index;
    {
        ...
        MX_LOG_DEBUG("sql.sql", "[%u ms] SQL(p): %s", getMSTimeDiff(_s, getMSTime()), m_mStmt->getQueryString(m_queries[index].first).c_str());
    ...
    for (itr = queries.begin(); itr != queries.end(); ++itr)
    {
        ...
        switch (itr->type)
        {
            case SQL_ELEMENT_PREPARED:
            {
                ...
                if (!Execute(stmt))
                {
                    MX_LOG_WARN("sql.sql", "Transaction aborted. %u queries not executed.", (uint32)queries.size());
                    RollbackTransaction();
                    return false;
                }
            }
...
MySQLPreparedStatement* MySQLConnection::GetPreparedStatement(uint32 index)
{
    ...
    if (!ret)
        MX_LOG_ERROR("sql.sql", "Could not fetch prepared statement %u on database `%s`, connection type: %s.",
...
void MySQLConnection::PrepareStatement(uint32 index, const char* sql, ConnectionFlags flags)
{
    ...
    if (!stmt)
    {
        MX_LOG_ERROR("sql.sql", "In mysql_stmt_init() id: %u, sql: \"%s\"", index, sql);
        MX_LOG_ERROR("sql.sql", "%s", mysql_error(m_Mysql));
        m_prepareError = true;
    }
    else
    {
        ...
        {
            MX_LOG_ERROR("sql.sql", "In mysql_stmt_prepare() id: %u, sql: \"%s\"", index, sql);
            MX_LOG_ERROR("sql.sql", "%s", mysql_stmt_error(stmt));
...
bool MySQLConnection::_HandleMySQLErrno(uint32 errNo)
{
    switch (errNo)
    {
        ...
        case CR_SERVER_LOST_EXTENDED:
        {
            ...
            {
                MX_LOG_INFO("sql.sql", "Connection to the MySQL server is active.");
                if (oldThreadId != mysql_thread_id(GetHandle()))
                    MX_LOG_INFO("sql.sql", "Successfully reconnected to %s @%s:%s (%s).",
        ...
        case ER_NO_SUCH_TABLE:
            MX_LOG_ERROR("sql.sql", "Your database structure is not up to date. Please make sure you've executed all queries in the sql/updates folders.");
            ACE_OS::sleep(10);
            std::abort();
            return false;
        case ER_PARSE_ERROR:
            MX_LOG_ERROR("sql.sql", "Error while parsing SQL. Core fix required.");
            ACE_OS::sleep(10);
            std::abort();
            return false;
        default:
            MX_LOG_ERROR("sql.sql", "Unhandled MySQL errno %u. Unexpected behaviour possible.", errNo);
            return false;
    }
}
