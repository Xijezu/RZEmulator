...
public:
    XSocketMgr()
        ...
        , _socketApplicationSendBufferSize(65536)
        , _tcpNoDelay(true)
    {
    }
...
protected:
    NetworkThread *CreateThreads() const override { return new XSocketThread[GetNetworkThreadCount()]; }

private:
    // private, must not be called directly
    bool StartNetwork(NGemity::Asio::IoContext &ioContext, std::string const &bindIp, uint16_t port, int threadCount) override { return SocketMgr::StartNetwork(ioContext, bindIp, port, threadCount); }
