...
namespace PacketDeclaration
{
     ...
     */
    template<class T>
    inline typename std::enable_if<!std::is_fundamental<T>::value && !std::is_enum<T>::value, int>::type getSizeOf(const T &value, int version)
    {
...
#define DEFINITION_F_dynstring(name, hasNullTerminator, ...) \
    ...
    {                                                        \
        enum                                                 \
        {                                                    \
            addNullTerminator = !!hasNullTerminator          \
        };                                                   \
    };
...
#define DESERIALIZATION_F_DYNSTRING2(name, hasNullTerminator) buffer->readDynString(#name, name, name##_size, hasNullTerminator);
...
#define CREATE_PACKET(name_, id_) CREATE_STRUCT_IMPL(name_, 7, CREATE_PACKET_DEFINITION_HEADER(id_), CREATE_PACKET_SERIALIZATION_HEADER, CREATE_PACKET_DESERIALIZATION_HEADER)
