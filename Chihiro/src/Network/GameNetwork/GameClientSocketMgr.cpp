...
class GameClientSocketThread : public NetworkThread<XSocket>
{
    public:
        void SocketAdded(std::shared_ptr<XSocket> sock) override
        {
            sock->SetSendBufferSize(sGCSocketMgr.GetApplicationSendBufferSize());
            sock->SetSession(new WorldSession{sock.get()});
            sock->Start();
        }
...
{
    sGCSocketMgr.OnSocketOpen(std::forward<tcp::socket>(sock), threadIndex);
}

GameClientSocketMgr::GameClientSocketMgr() : BaseSocketMgr(), _socketSystemSendBufferSize(-1), _socketApplicationSendBufferSize(65536), _tcpNoDelay(true)
{
}

GameClientSocketMgr& GameClientSocketMgr::Instance()
{
    static GameClientSocketMgr instance;
    return instance;
}

bool GameClientSocketMgr::StartWorldNetwork(NGemity::Asio::IoContext& ioContext, std::string const& bindIp, uint16 port, int threadCount)
{
...
void GameClientSocketMgr::StopNetwork()
{
    BaseSocketMgr::StopNetwork();
}

void GameClientSocketMgr::OnSocketOpen(tcp::socket&& sock, uint32 threadIndex)
{
...
NetworkThread<XSocket> *GameClientSocketMgr::CreateThreads() const
{
    return new GameClientSocketThread[GetNetworkThreadCount()];
}
