...
void SignalHandler(std::weak_ptr<NGemity::Asio::IoContext> ioContextRef,
...
int main(int argc, char **argv)
{
  ...
  std::shared_ptr<NGemity::Asio::IoContext> ioContext =
      std::make_shared<NGemity::Asio::IoContext>();
  ...
  {
    NG_LOG_ERROR("server.authserver", "Authnetwork startup failed: %s:%d",
  ...
  {
    NG_LOG_ERROR("server.authserver", "Gamenetwork startup failed: %s:%d",
  ...
  signals.async_wait(std::bind(
      &SignalHandler, std::weak_ptr<NGemity::Asio::IoContext>(ioContext),
  ...
  dbPingTimer->async_wait(
      std::bind(&KeepDatabaseAliveHandler,
                std::weak_ptr<boost::asio::deadline_timer>(dbPingTimer),
...
{
  if (!error)
    if (std::shared_ptr<NGemity::Asio::IoContext> ioContext =
            ioContextRef.lock())
      ioContext->stop();
}
    ...
    int32_t dbPingInterval, boost::system::error_code const &error)
{
  if (!error)
  {
    if (std::shared_ptr<boost::asio::deadline_timer> dbPingTimer =
            dbPingTimerRef.lock())
    {
